...
PCClient.prototype._startMetrics = function _startMetrics() {
  ...
  client._metricsInterval = setInterval(function metricInterval() {
    client.pc.getStats(function getStatsCb(report) {
      ...
      for(var i = 0; i < reportResults.length; i++) {
        if(reportResults[i].id.indexOf('send') >= 0) {
          ...
          stats.audioInputLevelTX = reportResults[i].stat('audioInputLevel');
        }
        if(reportResults[i].id.indexOf('recv') >= 0) {
          ...
          stats.audioInputLevelRX = reportResults[i].stat('audioInputLevel');
        }
      }
      ...
      dMetrics('PeerConnection Metrics',
        ...
        'RTT', stats.googRtt,
        'Audio Input TX', stats.audioInputLevelTX,
        'Audio Input RX', stats.audioInputLevelRX);
      stats = {};
    }, client._getStatsError);
  }, 1000);
};

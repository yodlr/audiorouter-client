...
WSClient.prototype._setupEventHandlers = function _setupEventHandlers() {
  var client = this;
  ...
  if(BROWSER) {
    ...
    client._ws.on('message', function onMessage(data, flags) {
      if(data) {
  ...
  if(client._reconnect) {
    if(client.wsReconnecting) {
      if(client._reconnectAttempts > 0) {
        ...
        setTimeout(function connectTimeout() {
          client.connect(); // TODO
        }, 1000);
      }
      else {
